...
> 2026-07-04 変更: APM 配布(`apm install HukuKaich0u/agent-kit/plugins/deep-research/skills/deep-research`)で skill を自己完結にするため、`scripts/`、`prompts/`、`schemas/`、`tests/`、`package.json` を `skills/deep-research/` 配下へ移動した。相対パス構造は下図のまま skill ディレクトリ内で維持される。`.codex-plugin/plugin.json` は変更なし。
