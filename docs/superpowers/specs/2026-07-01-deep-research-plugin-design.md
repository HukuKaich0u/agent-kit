...
> 2026-07-04 変更: APM 配布(`apm install HukuKaich0u/agent-kit/plugins/deep-research/skills/deep-research`)で skill を自己完結にするため、`scripts/`、`prompts/`、`schemas/`、`tests/`、`package.json` を `skills/deep-research/` 配下へ移動した(commit dc66863)。`.codex-plugin/plugin.json` は plugin root に残る。以下は移動後の実際のディレクトリ構成。
...
--keep-going              recoverable failure 後も継続。既定 true。互換のため --keep-going true|false も受理する
--no-keep-going           recoverable failure 発生時点で停止する
...
`status` は実装が実際に発行する `running | succeeded | failed | retrying` とする。
...
resume は phase 単位で行う。各 phase の出力 artifact(`scope.json`、`sources.json`/`claims.json`、`verification.json` など)を読み直し、schema valid ならその phase を再利用し、invalid または欠落していればその phase 以降を再実行する。個々の worker の実行状態(`workers/<worker-id>/execution.json`)は検証や復旧の対象ではなく、`running` のまま中断した worker を stale として個別に `failed` 回収する仕組みは実装していない。
