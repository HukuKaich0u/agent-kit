...
1. Retrieve the page. If you cannot access its content, set `extractionStatus` to "inaccessible" with empty `excerpts` and `claims`. If the page is unrelated to the research question, use "irrelevant".
2. Fill metadata honestly: resolvedUrl (final URL you actually read), title, publisher, publisherGroup (only when the owning group is verifiable from the page or clear publisher identity, else null), originUrl (only when the page explicitly links or attributes an original announcement/first publication, else null), author, publishedAt, retrievedAt = {{AS_OF}}, sourceType, quality with qualityRationale.
