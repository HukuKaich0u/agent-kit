<!-- apm:readme:begin (generated by scripts/gen-skill-readme.rb — do not edit this block) -->
# aws-ecs-codedeploy-blue-green

> ECS blue/green deployment patterns. Recommends ALB-native weighted target groups as the simpler default; covers CodeDeploy as a reference for teams that already use it or need pipeline-integrated rollback. Use when choosing or debugging ECS blue/green infrastructure.

## Install

### APM (recommended)

```bash
apm install HukuKaich0u/agent-kit/skills/aws/ecs-codedeploy-blue-green
```

Or declare it in `apm.yml` for a reproducible setup:

```yaml
dependencies:
  apm:
    - HukuKaich0u/agent-kit/skills/aws/ecs-codedeploy-blue-green
```

then run `apm install`.

### Any agentskills.io-compatible agent

This is an [Agent Skills](https://agentskills.io) skill — a portable
`SKILL.md` folder that works with Claude Code, Codex, opencode, Gemini CLI,
Cursor, and other [compatible agents](https://agentskills.io/clients).

With the [`skills`](https://github.com/vercel-labs/skills) CLI (installs into
every detected agent's skills directory):

```bash
npx skills add HukuKaich0u/agent-kit --skill aws-ecs-codedeploy-blue-green
```

Or copy the folder in directly:

```bash
npx degit HukuKaich0u/agent-kit/skills/aws/ecs-codedeploy-blue-green ~/.claude/skills/aws-ecs-codedeploy-blue-green
```

See [SKILL.md](./SKILL.md) for what this skill does and when to use it.
<!-- apm:readme:end -->
