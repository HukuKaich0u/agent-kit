<!-- apm:readme:begin (generated by scripts/gen-skill-readme.rb — do not edit this block) -->
# cloudflare-workers-otel-utels

> Cloudflare Worker telemetry at the fetch boundary — OTLP traces / metrics / logs + utels error tracking + D1 Proxy that emits slow-query warnings. Use when adding observability to a Worker without touching handler code.

## Install

### APM (recommended)

```bash
apm install HukuKaich0u/agent-kit/skills/cloudflare/workers-otel-utels
```

Or declare it in `apm.yml` for a reproducible setup:

```yaml
dependencies:
  apm:
    - HukuKaich0u/agent-kit/skills/cloudflare/workers-otel-utels
```

then run `apm install`.

### Any agentskills.io-compatible agent

This is an [Agent Skills](https://agentskills.io) skill — a portable
`SKILL.md` folder that works with Claude Code, Codex, opencode, Gemini CLI,
Cursor, and other [compatible agents](https://agentskills.io/clients).

With the [`skills`](https://github.com/vercel-labs/skills) CLI (installs into
every detected agent's skills directory):

```bash
npx skills add HukuKaich0u/agent-kit --skill cloudflare-workers-otel-utels
```

Or copy the folder in directly:

```bash
npx degit HukuKaich0u/agent-kit/skills/cloudflare/workers-otel-utels ~/.claude/skills/cloudflare-workers-otel-utels
```

See [SKILL.md](./SKILL.md) for what this skill does and when to use it.
<!-- apm:readme:end -->
