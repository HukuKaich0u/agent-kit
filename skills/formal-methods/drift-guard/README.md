<!-- apm:readme:begin (generated by scripts/gen-skill-readme.rb — do not edit this block) -->
# formal-methods-drift-guard

> Use after a formal model or verifier check already exists and the user wants to maintain it over time. Helps Codex compare specs/docs, implementation code/tests/config/logs, and formal models or CI checks for drift; classify whether the drift is in the spec, code, model abstraction, harness, or unresolved domain decision; run or plan verifier checks; and translate SAT/UNSAT changes, counterexample traces, proof failures, stale models, and CI results into domain-language review questions and ledger entries.

## Install

### APM (recommended)

```bash
apm install HukuKaich0u/agent-kit/skills/formal-methods/drift-guard
```

Or declare it in `apm.yml` for a reproducible setup:

```yaml
dependencies:
  apm:
    - HukuKaich0u/agent-kit/skills/formal-methods/drift-guard
```

then run `apm install`.

### Any agentskills.io-compatible agent

This is an [Agent Skills](https://agentskills.io) skill — a portable
`SKILL.md` folder that works with Claude Code, Codex, opencode, Gemini CLI,
Cursor, and other [compatible agents](https://agentskills.io/clients).

With the [`skills`](https://github.com/vercel-labs/skills) CLI (installs into
every detected agent's skills directory):

```bash
npx skills add HukuKaich0u/agent-kit --skill formal-methods-drift-guard
```

Or copy the folder in directly:

```bash
npx degit HukuKaich0u/agent-kit/skills/formal-methods/drift-guard ~/.claude/skills/formal-methods-drift-guard
```

See [SKILL.md](./SKILL.md) for what this skill does and when to use it.
<!-- apm:readme:end -->
