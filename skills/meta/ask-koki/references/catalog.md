...
`ask-koki` が読む一次カタログ。**このリポジトリ (`HukuKaich0u/agent-kit`) が
現在保有している skill だけ**を載せる。外部レジストリの一覧ではない — 外部探索は
`skill-finder`(Phase 2)の仕事。
...
| Status | Skill | Install (`skills/<path>`) | Use when |
|---|---|---|---|
| ✅ | backend-review-triage | `backend/review-triage` | backend レビューの入口。repo 全体を分類し適用する domain lens を選ぶ(security lens は未整備 — 認可・secrets 観点は現状別途) |
| ✅ | backend-review-architecture | `backend/review-architecture` | 構造・依存方向・境界漏れ。4言語の依存グラフ手段つき、新規ツール導入は承認制 |
| ✅ | backend-review-concurrency | `backend/review-concurrency` | async / 並行 / batch の危険パターン(Rust/Tokio・Go・asyncio・Node 対応) |
| ✅ | backend-review-data-access | `backend/review-data-access` | N+1 / 過剰取得(SQLx/Diesel/SeaORM〜Prisma・DynamoDB まで対応) |
| ✅ | backend-review-transactions | `backend/review-transactions` | トランザクション整合性(PG/MySQL/SQLite・D1/DynamoDB の engine 差分表つき) |
...
audit script 依存は除去済みで read-only desk review として動く(2026-07-26 監査で確認)。入口は review-triage。
