...
Each record is an immutable, point-in-time account of an approved design. It must start with `created`, `author`, and `type: design-record` YAML frontmatter — plus `based_on_revision` (the git revision the design was based on) when available — followed by a notice that current truth lives in code, tests, ADRs, and the domain glossary.
