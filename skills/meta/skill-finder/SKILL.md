...
5. **waxa eval gate (mandatory).** Adoption without empirical evaluation is the failure mode this skill exists to prevent. For each shortlisted candidate:
   ...
   - **Do not use `waxa iterate` here.** The auto-iterate loop can mis-converge (it re-runs the same prompt without editing the skill and marks the same unresolved issue as "reseen", which reads as zero new-unclear — see `INVENTORY.md`). Instead: read the ledger's unclear-points, decide adoption vs reject from a single run, and surface the decision for human approval rather than looping automatically.
...
| Skipping the pre-flight catalog check | Always check `skill-selector/references/catalog.md` first. If it covers the need, defer immediately — do not proceed into Tier 1+ surveys. The `playwright-test` / `tdd` / `gh-fix-ci` / `diagnosing-bugs` rows are the most common false-escalations to watch for. |
