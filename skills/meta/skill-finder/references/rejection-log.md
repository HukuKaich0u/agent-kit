...
- **Non-redundancy**: **FAILS** — overlaps with the existing skill stack in this repo:
...
- **Project-pin escape hatch**: a project that produces large numbers of skills *and* has no `superpowers` plugin / `empirical-prompt-tuning` skill installed could project-pin this with `apm install <repo>/skills/skill-creator#b9e19e6f4477`. This repo's typical stack has all three coverage skills, so this exception is unlikely.
- **Re-evaluate trigger**: only if one or more of `superpowers:writing-skills` / `empirical-prompt-tuning` / `waxa-eval` is removed from the global stack. Otherwise this entry stands indefinitely.
