...
2. **Plan** — identify shapes, relationships, layout (LR or TB), group by tier/layer. **Write the coordinate plan before any XML**: a lane table of columns (x centers) and rows (y). Estimate label widths first — a fullwidth (CJK) char ≈ fontSize px, ASCII ≈ 0.6×fontSize — and derive shape sizes and pitches from them, not the other way round. **Hubs get their corridor planned here, not patched after the first export**: for every hub (a sink ≥3 edges share — monitoring/logging, event bus, shared DB), the plan names the hub's position (event bus → center of the row it serves; monitoring/logging sink → the margin nearest most of its sources) and a trunk corridor with a concrete coordinate band (e.g. 「監視トレンチ: y=560–590」) that crosses no node box and no icon label zone, then lists each edge's exit port and corridor entry. **The plan is complete only when every edge traces source port → corridor → entry without crossing a node band** — an edge that can't be traced means a node or the corridor moves now, while moving is free. Corridor mechanics (trunk + staggered hub entries): `references/xml-authoring.md` → "Routing corridors". For vendor-icon diagrams (AWS/Azure/GCP) apply the spacing constants in `references/aws-architecture.md`
