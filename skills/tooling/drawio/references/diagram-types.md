...
| Element | Style | Notes |
|---------|-------|-------|
| Layer/tier | `rounded=0;whiteSpace=wrap;html=1;fillColor=<containerFill>;strokeColor=<tier stroke>;fontColor=<tier stroke>;verticalAlign=top;align=left;spacingLeft=8;fontStyle=1;fontSize=14;container=1;pointerEvents=0;` | **Tint ladder**: container fill = the tier hue's `containerFill` (~40% toward white), NOT the node fill — nodes must pop against the container. Title top-left. Alternate tint→white when nesting. See references/xml-authoring.md "Tint ladder" |
