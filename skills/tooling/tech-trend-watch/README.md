<!-- apm:readme:begin (generated by scripts/gen-skill-readme.rb — do not edit this block) -->
# tech-trend-watch

> Long-term technology stack review using State of JS, State of CSS, and Thoughtworks Technology Radar — satisfaction×usage matrix, ADOPT/TRIAL/ASSESS/HOLD mapping, P0–P3 migration priority. Use for annual stack audits, planning major migrations (jest→vitest, webpack→Vite, etc.), or on-demand replacement decisions. Not for general architecture design.

## Install

### APM (recommended)

```bash
apm install HukuKaich0u/agent-kit/skills/tooling/tech-trend-watch
```

Or declare it in `apm.yml` for a reproducible setup:

```yaml
dependencies:
  apm:
    - HukuKaich0u/agent-kit/skills/tooling/tech-trend-watch
```

then run `apm install`.

### Any agentskills.io-compatible agent

This is an [Agent Skills](https://agentskills.io) skill — a portable
`SKILL.md` folder that works with Claude Code, Codex, opencode, Gemini CLI,
Cursor, and other [compatible agents](https://agentskills.io/clients).

With the [`skills`](https://github.com/vercel-labs/skills) CLI (installs into
every detected agent's skills directory):

```bash
npx skills add HukuKaich0u/agent-kit --skill tech-trend-watch
```

Or copy the folder in directly:

```bash
npx degit HukuKaich0u/agent-kit/skills/tooling/tech-trend-watch ~/.claude/skills/tech-trend-watch
```

See [SKILL.md](./SKILL.md) for what this skill does and when to use it.
<!-- apm:readme:end -->
